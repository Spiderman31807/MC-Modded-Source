...
public class BorderData {
	...
	public void putWithinBounds(Player player, Entity target) {
		...
		while(target.level().getBlockState(BlockPos.containing(xPos, yPos, zPos)).isSuffocating(target.level(), BlockPos.containing(xPos, yPos, zPos))) {
			yPos++;
		}
		
		if(this.getDistance(target) > Math.max(1, this.getSpeed(target)))
