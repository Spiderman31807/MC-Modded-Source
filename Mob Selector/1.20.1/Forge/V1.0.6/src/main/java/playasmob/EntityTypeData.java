...
import net.minecraft.world.damagesource.DamageTypes;public class EntityTypeData {
	public MobData data;
	...
	public RegenModifer modifyRegen() {
		return this.getDiet() == Diet.None ? RegenModifer.Constant : RegenModifer.None;
	}
	...
	public boolean isInvulnerableTo(DamageSource source) {
		return source.is(DamageTypes.STARVE) && this.straveImmune();
	}
	...
	public void preTick() {
		...
		if(foodData.getFoodLevel() < this.minimumFoodLevel() && this.minimumFoodLevel() > 0)
			foodData.setFoodLevel(this.minimumFoodLevel());
		if(foodData.getFoodLevel() > this.maximumFoodLevel() && this.maximumFoodLevel() < 20)
			foodData.setFoodLevel(this.maximumFoodLevel());
	}
	...
	public boolean isFood(ItemStack stack) {
		if(!this.vaildFood(stack.getItem()))
			return false;
	...
	public boolean straveImmune() {
		return this.getDiet() == Diet.None;
	}
	...
	public int minimumFoodLevel() {
		return this.getDiet() == Diet.None ? 20 : 0;
	}
	...
	public void buildFoodInfo() {
		if(this.getDiet() == Diet.Custom || this.getDiet() == Diet.None)
			return;
			
		List<Item> dietItems = GlobalData.getItemTagged(ItemTags.create(new ResourceLocation("playasmob:food_" + this.dietName())));
		for(Item food : dietItems) {
			if(food.isEdible() || !this.vaildFood(food))
				continue;
			addFoodInfo(food, FoodMapper.get(food), null);
		}
	}

	public void addFoodInfo(Item item, @Nullable FoodProperties info, @Nullable SoundEvent sound) {
		if(item.isEdible() && info == null)
	...
	public void ate(ItemStack stack) {
		if(this instanceof BreedableData breedable && !data.player.getFoodData().needsFood())
	...
	public InteractionResult interactOn(Entity entity, InteractionHand hand) {
		if(data.player.getItemInHand(hand) == ItemStack.EMPTY) {
			if(data.world instanceof ServerLevel server && this instanceof BreedableData breedable && breedable.canMate(entity)) {
				breedable.spawnChildFromBreeding(server, entity);
			} else if(this.canRide(entity)) {
