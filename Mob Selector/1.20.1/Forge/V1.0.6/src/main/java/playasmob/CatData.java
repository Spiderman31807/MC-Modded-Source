...
public class CatData extends EntityTypeData implements TamableData, BreedableData {
	...
	public CatVariant variant = BuiltInRegistries.CAT_VARIANT.getOrThrow(CatVariant.BLACK);
   	...
   	public List<MobAttribute> getAbilities(EntityType type) {
   		...
   		abilities.add(MobAttribute.ability(1, Component.translatable("playasmob.ability.sit")).fullOnly());
   		abilities.add(MobAttribute.ability(2, Component.translatable("playasmob.ability.lay")).fullOnly());
   	...
   	public List<MobAttribute> getInfo(EntityType type) {
   		...
   		info.add(MobAttribute.info(Component.translatable("playasmob.info.piscivore")).tooltip(Component.translatable("playasmob.tooltip.piscivore")));
   		info.add(MobAttribute.info(Component.translatable("playasmob.info.tamable")).fullOnly());
   		...
   		info.add(MobAttribute.info(Component.translatable("playasmob.info.variants")).fullOnly());
   		return info;
   	}
	...
	public void load(CompoundTag compound) {
		...
		this.sleepYaw = compound.getFloat("sleepYaw");
	...
	public CompoundTag save() {
		...
		if(this.isLying()) {
			...
			compound.putFloat("sleepYaw", this.sleepYaw);
		}
	...
	public double speedMultiplier() {
		if(this.isLyingTransion() || this.isInSittingPose())
			return 0;
		if(data.isSprinting)
			return 0.5;
		if(data.isCrouching)
	...
	public boolean canSprint() {
		return super.canSprint() || (!this.isInSittingPose() && !this.isLyingTransion());
	}

   	@Override
	public boolean canCrouch() {
		return !this.isInSittingPose() && !this.isLyingTransion();
	}
   	...
   	public boolean isLyingTransion() {
		if(!data.fullyMobMode)
			return false;
	...
	public float sleepingYaw() {
		return -this.sleepYaw;
	}

   	public void setLying(boolean lying) {
		if(lying && !data.fullyMobMode)
			return;
      	this.isLying = lying;
      	if(lying)
      		this.sleepYaw = data.player.yBodyRot + 90;
   	}

   	public boolean isLying() {
      	return this.isLying && data.fullyMobMode;
   	}

   	public void setRelaxStateOne(boolean relax) {
		if(relax && !data.fullyMobMode)
			return;
      	this.relaxStateOne = relax;
  	}

   	public boolean isRelaxStateOne() {
      	return this.relaxStateOne && data.fullyMobMode;
   	}
