package playasmob;

import net.minecraftforge.event.entity.living.LivingFallEvent;

import java.util.ArrayList;
import java.util.List;

import net.minecraft.util.Mth;
import net.minecraft.client.renderer.entity.EntityRendererProvider;
import net.minecraft.world.entity.Pose;
import net.minecraft.world.entity.EntityDimensions;
import net.minecraft.world.item.ItemStack;
import net.minecraft.world.entity.animal.*;
import net.minecraft.world.entity.ai.attributes.AttributeSupplier;
import net.minecraft.client.renderer.entity.EntityRenderer;
import net.minecraft.world.entity.EntityType;
import net.minecraft.world.item.DyeColor;
import net.minecraft.nbt.CompoundTag;
import net.minecraft.server.level.ServerLevel;
import net.minecraft.world.entity.LightningBolt;
import net.minecraft.world.phys.Vec3;
import net.minecraft.sounds.SoundEvents;
import net.minecraft.world.level.gameevent.GameEvent;
import net.minecraft.world.item.Items;
import net.minecraft.network.chat.Component;
import net.minecraft.world.damagesource.DamageSource;
import net.minecraft.sounds.SoundEvent;
import net.minecraft.client.gui.GuiGraphics;
import net.minecraft.world.item.Item;
import net.minecraft.world.entity.Entity;
import net.minecraft.world.inventory.CraftingContainer;
import javax.annotation.Nullable;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.inventory.AbstractContainerMenu;
import net.minecraft.world.item.crafting.RecipeType;
import net.minecraft.world.item.DyeItem;
import net.minecraft.world.inventory.TransientCraftingContainer;

public class AnimalData extends EntityTypeData implements AgeableData, BreedableData {
	public AgeData ageData;
	public BreedData breedData;
   	public int eatAnimationTick = 0;
   	public boolean isSheared = false;
   	public DyeColor color = DyeColor.WHITE;
   	public int eggTime = 6000;
   	public float flapping = 1.0F;
   	public float nextFlap = 1.0F;
   	public float oFlapSpeed;
  	public float flapSpeed;
   	public float oFlap;
   	public float flap;

   	@Override
   	public List<MobAttribute> getAbilities(EntityType type) {
   		ArrayList<MobAttribute> abilities = new ArrayList();
   		if(type == EntityType.CHICKEN)
   			abilities.add(MobAttribute.ability(1, Component.translatable("playasmob.ability.lay_egg")));
   		return abilities;
   	}

   	@Override
   	public List<MobAttribute> getPros(EntityType type) {
   		ArrayList<MobAttribute> pros = new ArrayList();
   		if(type == EntityType.CHICKEN)
   			pros.add(MobAttribute.pro(Component.translatable("playasmob.pro.slow_fall")));
   		return pros;
   	}

   	@Override
   	public List<MobAttribute> getInfo(EntityType type) {
   		ArrayList<MobAttribute> info = new ArrayList();
   		info.add(MobAttribute.info(Component.translatable("playasmob.info." + (type == EntityType.CHICKEN ? "granivore" : "herbivore"))).tooltip(Component.translatable("playasmob.tooltip." + (type == EntityType.CHICKEN ? "granivore" : "herbivore"))));
   		info.add(MobAttribute.info(Component.translatable("playasmob.info.ageable")).fullOnly());
   		info.add(MobAttribute.info(Component.translatable("playasmob.info.breedable")).fullOnly());
   		if(type == EntityType.PIG)
   			info.add(MobAttribute.info(Component.translatable("playasmob.info.transform_pig")).fullOnly());
   		return info;
   	}

   	@Override
   	public List<MobAttribute> getCons(EntityType type) {
   		ArrayList<MobAttribute> cons = new ArrayList();
   		cons.add(MobAttribute.con(Component.translatable("playasmob.con.health")));
   		cons.add(MobAttribute.con(Component.translatable("playasmob.con.hand")).fullOnly());
   		cons.add(MobAttribute.con(Component.translatable("playasmob.con.pickup")).fullOnly());
   		return cons;
   	}
	
	public AnimalData(MobData data) {
		super(data);
		this.ageData = new AgeData(this, data);
		this.breedData = new BreedData(this, data);
	}

   	@Override
	public void load(CompoundTag compound) {
		if(compound.contains("AgeData"))
			this.ageLoad(compound.getCompound("AgeData"));
		if(compound.contains("BreedData"))
			this.breedLoad(compound.getCompound("BreedData"));

		switch(GlobalUtils.getString(data.mob)) {
			default:
				break;
			case "minecraft:chicken":
				this.eggTime = compound.getInt("EggLayTime");
				this.oFlapSpeed = compound.getFloat("oFlapSpeed");
				this.flapSpeed = compound.getFloat("flapSpeed");
				this.oFlap = compound.getFloat("oFlap");
				this.flap = compound.getFloat("flap");
      			break;
		};
	}

   	@Override
	public CompoundTag save() {
		CompoundTag compound = new CompoundTag();
		compound.put("AgeData", this.ageSave());
		compound.put("BreedData", this.breedSave());

		switch(GlobalUtils.getString(data.mob)) {
			default:
				break;
			case "minecraft:chicken":
      			compound.putInt("EggLayTime", this.eggTime);
      			compound.putFloat("oFlapSpeed", this.oFlapSpeed);
      			compound.putFloat("flapSpeed", this.flapSpeed);
      			compound.putFloat("oFlap", this.oFlap);
      			compound.putFloat("flap", this.flap);
      			break;
		};
		
		return compound;
	}

   	@Override
	public AgeData ageData() {
		return this.ageData;
	}

   	@Override
	public BreedData breedData() {
		return this.breedData;
	}

   	@Override
   	public boolean canBreed() {
      	return data.fullyMobMode;
   	}
	
   	@Nullable
   	@Override
   	public Animal getBreedOffspring(ServerLevel server, Entity partner) {
      	Animal animal = (Animal)data.mob.create(server);
      	if(animal instanceof Sheep babySheep && partner instanceof Sheep sheep)
			babySheep.setColor(this.getOffspringColor(this.getColor(), sheep.getColor()));
      	if(animal instanceof Sheep babySheep && partner instanceof Player player && MobData.get(player).typeData instanceof AnimalData animalData)
			babySheep.setColor(this.getOffspringColor(this.getColor(), animalData.getColor()));
      	return animal;
   	}

   	public DyeColor getOffspringColor(DyeColor color1, DyeColor color2) {
      	CraftingContainer craftingcontainer = makeContainer(color1, color2);
      	return data.world.getRecipeManager().getRecipeFor(RecipeType.CRAFTING, craftingcontainer, data.world).map((p_289444_) -> {
         	return p_289444_.assemble(craftingcontainer, data.world.registryAccess());
      	}).map(ItemStack::getItem).filter(DyeItem.class::isInstance).map(DyeItem.class::cast).map(DyeItem::getDyeColor).orElseGet(() -> {
         	return data.world.random.nextBoolean() ? color1 : color2;
      	});
   	}

  	public static CraftingContainer makeContainer(DyeColor color1, DyeColor color2) {
      	CraftingContainer craftingcontainer = new TransientCraftingContainer(new AbstractContainerMenu(null, -1) {
         	public ItemStack quickMoveStack(Player p_218264_, int p_218265_) { return ItemStack.EMPTY; }
         	public boolean stillValid(Player p_29888_) {return false; }
      	}, 2, 1);
      	craftingcontainer.setItem(0, new ItemStack(DyeItem.byColor(color1)));
      	craftingcontainer.setItem(1, new ItemStack(DyeItem.byColor(color2)));
      	return craftingcontainer;
   	}

   	@Override
	public void renderInfo(GuiGraphics graphics, int width, int height) {
		if(data.mob == EntityType.CHICKEN)
			RenderUtils.abilityIcon(graphics, "egg", true, width - 30, height - 22, this.eggTime, Keybinds.Ability1);
	}
	
   	@Override
   	public void preTick() {
		super.preTick();
   		if(this.ageData != null)
   			this.ageData.tick();
   		if(this.breedData != null)
   			this.breedData.tick();

   		switch(GlobalUtils.getString(data.mob)) {
			case "minecraft:chicken" -> this.chickenTick();
		};
   	}

   	public void chickenTick() {
      	this.oFlap = this.flap;
      	this.oFlapSpeed = this.flapSpeed;
      	this.flapSpeed += (data.player.onGround() ? -1.0F : 4.0F) * 0.3F;
      	this.flapSpeed = Mth.clamp(this.flapSpeed, 0.0F, 1.0F);
      	if (!data.player.onGround() && this.flapping < 1.0F)
         	this.flapping = 1.0F;

      	this.flapping *= 0.9F;
      	Vec3 vec3 = data.player.getDeltaMovement();
      	if (!data.player.onGround() && vec3.y < 0.0D)
         	data.player.setDeltaMovement(vec3.multiply(1.0D, 0.6D, 1.0D));

      	this.flap += this.flapping * 2.0F;
      	if(this.eggTime > 0)
      		this.eggTime--;
   	}

   	@Override
   	public void press(int ability, boolean pressed) {
   		if(data.mob == EntityType.CHICKEN) {
   			if(ability == 1 && pressed)
   				this.layEgg();
   		}
   	}

   	public void layEgg() {
   		if(data.world != null && data.player.isAlive() && !this.isBaby() && this.eggTime <= 0) {
        	data.player.playSound(SoundEvents.CHICKEN_EGG, 1, (data.world.random.nextFloat() - data.world.random.nextFloat()) * 0.2F + 1.0F);
			if(!data.world.isClientSide)
        		data.player.spawnAtLocation(Items.EGG);
        	data.player.gameEvent(GameEvent.ENTITY_PLACE);
       		this.eggTime = data.world.random.nextInt(6000) + 6000;
       		data.sync(false, false);
      	}
   	}

   	public boolean isFlapping() {
      	return data.player.flyDist > this.nextFlap;
   	}

   	public void onFlap() {
      	this.nextFlap = data.player.flyDist + this.flapSpeed / 2.0F;
   	}

   	@Override
   	public void thunderHit(ServerLevel server, LightningBolt bolt) {
      	if(data.mob == EntityType.PIG && data.fullyMobMode) {
      		data.changeMob(EntityType.ZOMBIFIED_PIGLIN, null, false);
      		return;
      	}
   	}

   	@Override
	public boolean canSprint() {
		return true;
	}
	
   	@Override
	public void causeFallDamage(LivingFallEvent event) {
		if(data.mob == EntityType.CHICKEN)
			event.setCanceled(true);
	}

   	@Override
	public AttributeSupplier.Builder modifyAttributes(AttributeSupplier.Builder playerBuilder) {
		AttributeSupplier.Builder animalBuilder = switch(GlobalUtils.getString(data.mob)) {
			default -> null;
			case "minecraft:cow" -> Cow.createAttributes();
			case "minecraft:sheep" -> Sheep.createAttributes();
			case "minecraft:pig" -> Pig.createAttributes();
			case "minecraft:chicken" -> Chicken.createAttributes();
		};

		if(animalBuilder != null)
			playerBuilder.combine(animalBuilder);
		return playerBuilder;
	}

   	@Override
   	public SoundEvent getAmbientSound() {
		return switch(GlobalUtils.getString(data.mob)) {
			default -> null;
			case "minecraft:cow" -> SoundEvents.COW_AMBIENT;
			case "minecraft:sheep" -> SoundEvents.SHEEP_AMBIENT;
			case "minecraft:pig" -> SoundEvents.PIG_AMBIENT;
			case "minecraft:chicken" -> SoundEvents.CHICKEN_AMBIENT;
		};
   	}

   	@Override
	public SoundEvent getHurtSound(DamageSource source) {
		return switch(GlobalUtils.getString(data.mob)) {
			default -> null;
			case "minecraft:cow" -> SoundEvents.COW_HURT;
			case "minecraft:sheep" -> SoundEvents.SHEEP_HURT;
			case "minecraft:pig" -> SoundEvents.PIG_HURT;
			case "minecraft:chicken" -> SoundEvents.CHICKEN_HURT;
		};
	}

   	@Override
	public SoundEvent getDeathSound() {
		return switch(GlobalUtils.getString(data.mob)) {
			default -> null;
			case "minecraft:cow" -> SoundEvents.COW_DEATH;
			case "minecraft:sheep" -> SoundEvents.SHEEP_DEATH;
			case "minecraft:pig" -> SoundEvents.PIG_DEATH;
			case "minecraft:chicken" -> SoundEvents.CHICKEN_DEATH;
		};
	}

   	@Override
   	public SoundEvent getStepSound() {
		return switch(GlobalUtils.getString(data.mob)) {
			default -> null;
			case "minecraft:cow" -> SoundEvents.COW_STEP;
			case "minecraft:sheep" -> SoundEvents.SHEEP_STEP;
			case "minecraft:pig" -> SoundEvents.PIG_STEP;
			case "minecraft:chicken" -> SoundEvents.CHICKEN_STEP;
		};
   	}

   	@Override
   	public float stepVolume() {
   		return 0.15f;
   	}

   	@Override
	public double speedMultiplier() {
		if(data.isSprinting)
			return 0.4;
		
		return switch(GlobalUtils.getString(data.mob)) {
			default -> 1;
			case "minecraft:cow" -> 0.3;
			case "minecraft:sheep" -> 0.38;
			case "minecraft:pig" -> 0.4;
			case "minecraft:chicken" -> 0.4;
		};
	}

   	@Override
	public EntityRenderer getRenderer(EntityRendererProvider.Context context) {
		return switch(GlobalUtils.getString(data.renderAs)) {
			default -> null;
			case "minecraft:cow" -> new CowRenderer(context);
			case "minecraft:sheep" -> new SheepRenderer(context);
			case "minecraft:pig" -> new PigRenderer(context);
			case "minecraft:chicken" -> new ChickenRenderer(context);
		};
	}

   	@Override
	public float getEyeHeight(Pose pose, EntityDimensions dimensions) {
		return dimensions.height * switch(GlobalUtils.getString(data.mob)) {
			default -> 0.85f;
			case "minecraft:cow" -> this.isBaby() ? 0.95f : 0.925f;
			case "minecraft:sheep" -> 0.95f;
			case "minecraft:chicken" -> this.isBaby() ? 0.85f : 0.92f;
		};
	}

   	public boolean isSheared() {
      	return this.isSheared;
   	}

   	public DyeColor getColor() {
      	return this.color;
   	}
   
   	public float getHeadEatPositionScale(float p_29881_) {
      	if (this.eatAnimationTick <= 0) {
         	return 0.0F;
      	} else if (this.eatAnimationTick >= 4 && this.eatAnimationTick <= 36) {
         	return 1.0F;
      	} else {
         	return this.eatAnimationTick < 4 ? ((float)this.eatAnimationTick - p_29881_) / 4.0F : -((float)(this.eatAnimationTick - 40) - p_29881_) / 4.0F;
      	}
   	}

   	public float getHeadEatAngleScale(float p_29883_) {
      	if (this.eatAnimationTick > 4 && this.eatAnimationTick <= 36) {
         	float f = ((float)(this.eatAnimationTick - 4) - p_29883_) / 32.0F;
         	return ((float)Math.PI / 5F) + 0.21991149F * Mth.sin(f * 28.7F);
      	} else {
         	return this.eatAnimationTick > 0 ? ((float)Math.PI / 5F) : data.player.getXRot() * ((float)Math.PI / 180F);
      	}
   	}

   	@Override
   	public Diet getDiet() {
   		return data.mob == EntityType.CHICKEN ? Diet.Granivore : Diet.Herbivore;
   	}

   	@Override
   	public void buildFoodInfo() {
   		super.buildFoodInfo();
   		if(data.mob == EntityType.COW || data.mob == EntityType.SHEEP)
   			this.addFoodInfo(Items.WHEAT, FoodMapper.create(5), null);
   	}

   	@Override
   	public boolean vaildFood(Item item) {
   		return super.vaildFood(item) && switch(GlobalUtils.getString(data.mob)) {
			default -> true;
			case "minecraft:cow" -> item == Items.WHEAT;
			case "minecraft:sheep" -> item == Items.WHEAT;
			case "minecraft:pig" -> List.of(Items.CARROT, Items.BEETROOT, Items.POTATO).contains(item);
		};
   	}

   	@Override
	public boolean canSwim() {
		return false;
	}

   	@Override
	public boolean canGlide() {
		return false;
	}
}
