...
public class SpiderData extends EntityTypeData implements GolemAttack {
	...
	public void setNightVision(boolean enabled) {
		...
		if(enabled) {
			data.player.forceAddEffect(new MobEffectInstance(MobEffects.NIGHT_VISION, -1, 0, false, false, false), null);
	...
	public void cycleClimbState(boolean direction) {
		if(direction) {
        	this.climbState = (this.climbState >= 1) ? -2 : this.climbState + 1;
    	} else {
        	this.climbState = (this.climbState <= -2) ? 1 : this.climbState - 1;
    	}
	}
   	...
   	public void postTick() {
        ...
        if(this.isClimbing()) {
        	...
        	Vec3 delta = data.player.getDeltaMovement();
       		boolean moving = delta.x != 0 || delta.z != 0;
       		boolean climb = this.isJumping() || (this.climbState == 1 && moving);
       		boolean descend = data.player.isCrouching() == this.climbState >= 0;
        	double yDelta = climb ? 0.2 : (descend ? -0.2 : 0);
        	if(yDelta >= 0 || this.climbState != -2) {
	...
	public float getEyeHeight(Pose pose, EntityDimensions dimensions) {
		return dimensions.height * (data.mob == EntityType.SPIDER ? 0.722f : 0.9f);
	}
	...
	public void hurtEntity(LivingHurtEvent event) {
   	...
   	public boolean vaildFood(Item item) {
   		return super.vaildFood(item) && item != Items.SPIDER_EYE;
   	}
