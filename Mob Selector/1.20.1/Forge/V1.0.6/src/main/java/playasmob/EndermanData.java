...
public class EndermanData extends EntityTypeData implements GolemAttack {
	...
	public boolean creepy = false;
   	...
   	public List<MobAttribute> getAbilities(EntityType type) {
   		...
   		abilities.add(MobAttribute.ability(3, Component.translatable("playasmob.ability.creepy_face")).fullOnly());
   		return abilities;
   	}
	...
	public CompoundTag save() {
		...
		if(this.creepy)
			compound.putBoolean("creepy", true);
		return compound;
	}
	...
	public void load(CompoundTag compound) {
		...
		this.creepy = compound.getBoolean("creepy");
	}
	...
	public void renderInfo(GuiGraphics graphics, int width, int height) {
		...
		if(data.fullyMobMode)
			RenderUtils.abilityIcon(graphics, "creepy_" + !this.creepy, true, width - 30, height - 58, -1, Keybinds.Ability3);
	}
	...
	public void press(int ability, boolean pressed) {
		...
		if(ability == 3 && data.fullyMobMode)
	...
	public float getEyeHeight(Pose pose, EntityDimensions dimensions) {
		return dimensions.height * 0.879f;
	}
	...
	public void preTick() {
   	...
   	public void buildFoodInfo() {
   		super.buildFoodInfo();
   		this.addFoodInfo(Items.CHORUS_FRUIT, null, null);
   	}
   	...
   	public boolean teleportForward() {
		...
		result = data.world.clip(new ClipContext(targetPosition, targetPosition.subtract(0, dimensions.height * 1.5, 0), ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, data.player));
		if(result.getType() != HitResult.Type.MISS)
			targetPosition = result.getLocation();
