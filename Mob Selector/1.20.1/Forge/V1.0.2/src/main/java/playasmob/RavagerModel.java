...
@OnlyIn(Dist.CLIENT)
public class RavagerModel extends HierarchicalModel<AbstractClientPlayer> {
   ...
   public void setupAnim(AbstractClientPlayer player, float p_103627_, float p_103628_, float p_103629_, float p_103630_, float p_103631_) {
      this.head.xRot = Mth.clamp(p_103631_ * ((float)Math.PI / 180F), -0.5f, 0.5f);
   ...
   public void prepareMobModel(AbstractClientPlayer player, float p_103622_, float p_103623_, float p_103624_) {
      super.prepareMobModel(player, p_103622_, p_103623_, p_103624_);
      int i = 0;
      int j = 0;
      int l = 0;
      if(MobData.get(player).typeData instanceof RaiderData data) {
	      i = data.getStunnedTick();
	      j = data.getRoarTick();
	      l = data.getAttackTick();
      }
