...
public class WardenData implements VibrationSystem.User, VibrationSystem, EntityTypeData, GolemAttack {
	...
	public CompoundTag save() {
		...
		boolean alive = data.player == null ? true : data.player.isAlive();
		if(this.sniffAnimation > 0 && (this.sniffAnimation - tick) > -60 && alive) {
			compound.putInt("sniffAnimation", this.sniffAnimation);
			if((this.sniffAnimation - tick) == -50)
				this.finishSniffing();
		} else if(this.sniffAnimation > 0 && (this.sniffAnimation - tick) <= -60) {
			this.sniffAnimation = 0;
			data.updateSpeed();
			data.updateReach();
		}

		if(this.roarAnimation > 0 && (this.roarAnimation - tick) > -80 && alive) {
			compound.putInt("roarAnimation", this.roarAnimation);
			if((this.roarAnimation - tick) == -60)
				this.finishRoaring();
		} else if(this.roarAnimation > 0 && (this.roarAnimation - tick) <= -80) {
			this.roarAnimation = 0;
			data.updateSpeed();
			data.updateReach();
		}

		if(this.sonicAnimation > 0 && (this.sonicAnimation - tick) > -50 && alive) {
			compound.putInt("sonicAnimation", this.sonicAnimation);
			if(this.sonicTarget != null)
				compound.putUUID("sonicTarget", this.sonicTarget);
			if((this.sonicAnimation - tick) == -34)
				this.finishSonicBoom();
		} else if(this.sonicAnimation > 0 && (this.sonicAnimation - tick) <= -50) {
			this.sonicAnimation = 0;
			data.updateSpeed();
			data.updateReach();
		}

		if(this.attackAnimation > 0 && (this.attackAnimation - tick) > -30 && alive) {
			compound.putInt("attackAnimation", this.attackAnimation);
		} else if(this.attackAnimation > 0 && (this.attackAnimation - tick) <= -30) {
	...
	public void preTick() {
        ...
        if (data.player.tickCount % this.getHeartBeatDelay() == 0) {
            ...
            data.player.playSound(SoundEvents.WARDEN_HEARTBEAT, 5.0F, data.player.getVoicePitch());
        }
