...
public class Events {
	...
	public static void entityRender(RenderLivingEvent.Pre event) {
		MobData clientData = MobData.get(Minecraft.getInstance().player);
		if(clientData.typeData == null)
			return;
			
		if(clientData.typeData.renderEntity(event.getEntity()) == false) {
			event.setCanceled(true);
			return;
		}
		
		LivingEntityRenderer renderer = event.getRenderer();
		removeGlowLayers(renderer);
		renderer.addLayer(new GlowLayer(renderer, getContext().getModelSet(), clientData.typeData));
	}

	public static void removeGlowLayers(LivingEntityRenderer renderer) {
        try {
            Field layersField = LivingEntityRenderer.class.getDeclaredField("layers");
            layersField.setAccessible(true);
            ArrayList<RenderLayer> newLayers = new ArrayList();
            for(RenderLayer layer : (List<RenderLayer>)layersField.get(renderer)) {
            	if(layer instanceof GlowLayer)
            		continue;
            	newLayers.add(layer);
            }
            layersField.set(renderer, newLayers);
