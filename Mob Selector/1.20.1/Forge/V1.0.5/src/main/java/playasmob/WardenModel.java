...
@OnlyIn(Dist.CLIENT)
public class WardenModel extends HierarchicalModel<AbstractClientPlayer> implements ArmedModel, HeadedModel {
   	...
   	public final List<ModelPart> tendrilsLayerModelParts;
   	public final List<ModelPart> heartLayerModelParts;
   	public final List<ModelPart> bioluminescentLayerModelParts;
   	public final List<ModelPart> pulsatingSpotsLayerModelParts;
   	...
   	public void setupAnim(AbstractClientPlayer player, float p_233532_, float p_233533_, float p_233534_, float p_233535_, float p_233536_) {
      	this.root().getAllParts().forEach(ModelPart::resetPose);
      	if(MobData.get(player).typeData instanceof WardenData warden) {
	      	float f = p_233534_ - (float)player.tickCount;
	      	this.animateHeadLookTarget(p_233535_, p_233536_);
	      	this.animateWalk(p_233532_, p_233533_);
	      	this.animateIdlePose(p_233534_);
	      	this.animateTendrils(player, p_233534_, f);
	      	this.animate(warden.attackAnimationState, WardenAnimation.WARDEN_ATTACK, p_233534_);
	      	this.animate(warden.sonicBoomAnimationState, WardenAnimation.WARDEN_SONIC_BOOM, p_233534_);
	      	this.animate(warden.diggingAnimationState, WardenAnimation.WARDEN_DIG, p_233534_);
	      	this.animate(warden.emergeAnimationState, WardenAnimation.WARDEN_EMERGE, p_233534_);
	    	this.animate(warden.roarAnimationState, WardenAnimation.WARDEN_ROAR, p_233534_);
			this.animate(warden.sniffAnimationState, WardenAnimation.WARDEN_SNIFF, p_233534_);
      	}
   	}
	...
	private void animateHeadLookTarget(float p_233517_, float p_233518_) {
   	...
   	private void animateTendrils(AbstractClientPlayer player, float p_233528_, float p_233529_) {
   		float f = 0;
   		if(MobData.get(player).typeData instanceof WardenData warden)
      		f = warden.getTendrilAnimation(p_233529_) * (float)(Math.cos((double)p_233528_ * 2.25D) * Math.PI * (double)0.1F);
