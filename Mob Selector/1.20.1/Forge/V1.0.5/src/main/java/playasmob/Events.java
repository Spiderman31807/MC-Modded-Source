...
public class Events {
	...
	public static void playerRender(RenderPlayerEvent.Pre event) {
		...
		if(data.renderAs == EntityType.PLAYER || !data.fullyMobMode)
			return;
	...
	public static void armRender(RenderArmEvent event) {
		...
		MobData data = MobData.get(player);
		if(data.typeData == null || !data.fullyMobMode)
			return;
	...
	public static void renderItemTooltip(ItemTooltipEvent event) {
		...
		MobData data = MobData.get(entity);
		if(data.typeData != null)
			data.typeData.addItemTooltip(event);
	}
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		...
		event.getDispatcher().register(Commands.literal("mob").requires(s -> s.hasPermission(2)).then(Commands.literal("set").then(Commands.argument("mob", ResourceArgument.resource(context, Registries.ENTITY_TYPE)).suggests(GlobalData.suggest()).executes(arguments -> {
			...
			if(player == null)
				return 0;
			MobData data = MobData.get(player);
			data.changeMob(EntityType.PLAYER, null, false);
			data.hasSpawned = false;
			MobSelectionMenu.openSelector(player);
			return 15;
		})).then(Commands.literal("age").then(Commands.argument("amount", IntegerArgumentType.integer()).executes(arguments -> {
			Player player = (Player)arguments.getSource().getEntity();
			if(player != null) {
				MobData data = MobData.get(player);
				if(data != null && data.typeData != null) {
					if(data.typeData instanceof AgeableData ageData) {
						ageData.setAge(IntegerArgumentType.getInteger(arguments, "amount"));
						data.sync(true, false);
					} else {
						data.typeData.setBaby(IntegerArgumentType.getInteger(arguments, "amount") < 0);
						data.sync(true, false);
					}
					return 15;
				}
			...
			return 0;
		}))).then(Commands.literal("fullMode").then(Commands.argument("toggle", BoolArgumentType.bool()).executes(arguments -> {
			Player player = (Player)arguments.getSource().getEntity();
			if(player != null) {
				MobData data = MobData.get(player);
				if(data != null && data.typeData != null) {
					data.fullyMobMode = BoolArgumentType.getBool(arguments, "toggle");
					data.sync(true, false);
					return 15;
				}
			}
	...
	public static void breathe(LivingBreatheEvent event) {
		if(event.getEntity() instanceof Player player) {
			MobData data = MobData.get(player);
			if(data != null && data.typeData != null)
				data.typeData.modifyBreathing(event);
		}
	}
	...
	public static void digSpeed(PlayerEvent.BreakSpeed event) {
		...
		if(data != null && data.typeData != null)
			data.typeData.modifyDigSpeed(event);
	}
	
	@SubscribeEvent
	public static void sleep(PlayerSleepInBedEvent event) {
		MobData data = MobData.get(event.getEntity());
		if(data != null && data.typeData != null && data.typeData.preventSleeping() == true)
	...
	public static void livingTick(LivingEvent.LivingTickEvent event) {
		if(event.getEntity() instanceof Player player) {
			MobData data = MobData.get(player);
	    	if(data != null && data.typeData != null && !data.typeData.hasWorldCollision()) {
	...
	@SubscribeEvent
	public static void defaultArrow(LivingGetProjectileEvent event) {
		if(event.getEntity() instanceof Player player) {
			MobData data = MobData.get(player);
			if(data != null && data.typeData != null && event.getProjectileItemStack().isEmpty())
				event.setProjectileItemStack(data.typeData.defaultArrow(event.getProjectileWeaponItemStack()));
		}
	}
}