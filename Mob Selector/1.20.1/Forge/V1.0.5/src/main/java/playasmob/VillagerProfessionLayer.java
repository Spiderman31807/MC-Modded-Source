...
@OnlyIn(Dist.CLIENT)
public class VillagerProfessionLayer<M extends EntityModel<AbstractClientPlayer> & VillagerHeadModel> extends RenderLayer<AbstractClientPlayer, M> {
   ...
   public VillagerProfessionLayer(RenderLayerParent<AbstractClientPlayer, M> p_174550_, ResourceManager p_174551_, String p_174552_) {
      super(p_174550_);
      this.resourceManager = p_174551_;
      this.path = p_174552_;
   }

   public void render(PoseStack p_117646_, MultiBufferSource p_117647_, int p_117648_, AbstractClientPlayer player, float p_117650_, float p_117651_, float p_117652_, float p_117653_, float p_117654_, float p_117655_) {
      if (!player.isInvisible()) {
         VillagerData villagerdata = getData(player).getVillagerData();
         ...
         renderColoredCutoutModel(m, resourcelocation, p_117646_, p_117647_, p_117648_, player, 1.0F, 1.0F, 1.0F);
         m.hatVisible(true);
         if (villagerprofession != VillagerProfession.NONE && !getData(player).isBaby()) {
            ...
            renderColoredCutoutModel(m, resourcelocation1, p_117646_, p_117647_, p_117648_, player, 1.0F, 1.0F, 1.0F);
            if (villagerprofession != VillagerProfession.NITWIT) {
               ...
               renderColoredCutoutModel(m, resourcelocation2, p_117646_, p_117647_, p_117648_, player, 1.0F, 1.0F, 1.0F);
            }
         }
   ...
   public playasmob.VillagerData getData(AbstractClientPlayer player) {
       if(MobData.get(player).typeData instanceof playasmob.VillagerData data)
           return data;
       return new playasmob.VillagerData(MobData.get(player));
   }
