...
public class GlobalUtils {
	...
	public static void dropHandItem(Player player) {
		dropItem(player, EquipmentSlot.MAINHAND);
	}
	...
	public static void randomiseEnchants(Player player) {
		RandomSource random = player.getRandom();
      	float multiplier = player.level().getCurrentDifficultyAt(player.blockPosition()).getSpecialMultiplier();
      	if(!player.getMainHandItem().isEmpty() && random.nextFloat() < 0.25F * multiplier)
         	player.setItemSlot(EquipmentSlot.MAINHAND, EnchantmentHelper.enchantItem(random, player.getMainHandItem(), (int)(5.0F + multiplier * (float)random.nextInt(18)), false));

      	for(EquipmentSlot slot : EquipmentSlot.values()) {
         	if (slot.getType() == EquipmentSlot.Type.ARMOR) {
				ItemStack stack = player.getItemBySlot(slot);
			    if(!stack.isEmpty() && random.nextFloat() < 0.5F * multiplier)
			    	player.setItemSlot(slot, EnchantmentHelper.enchantItem(random, stack, (int)(5.0F + multiplier * (float)random.nextInt(18)), false));
         	}
      	}
	}
	...
	public static boolean isRavager(Entity entity) {
		if(entity instanceof Player player)
			return MobData.get(player).typeData instanceof RaiderData typeData && typeData.data.mob == EntityType.RAVAGER;
