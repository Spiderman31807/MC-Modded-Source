...
public class MobSelectScreen extends AbstractContainerScreen<MobSelectionMenu> {
	...
	public boolean isPauseScreen() {
   	...
   	public void renderPreview(GuiGraphics graphics, MobPreview preview) {
   		this.renderSelection(graphics, preview.getSelection(), "select");
   	...
   	public void renderGroup(GuiGraphics graphics, SelectionGroup group) {
		...
		for(EntityType mob : group.mobs) {
         	...
         	if(selection.y > 0 && selection.y < this.height)
            	this.renderSelection(graphics, selection, "preview");
      	}
   	...
   	public void renderOptions(GuiGraphics graphics) {
        ...
        if(playerSelection.y > 0 && playerSelection.y < this.height)
      	...
      	for(SelectionGroup group : SelectionScreenData.groups) {
         	...
         	if(selection.y > 0 && selection.y < this.height)
            	this.renderSelection(graphics, selection, "openGroup");
      	}
   	...
   	public void renderSelection(GuiGraphics graphics, MobSelection selection, String message) {
   		this.addButton(selection, message);
   		MobBackground background = selection.getBackground();
   		renderEntityType(graphics, selection.mob, selection.x + (background.sizeX / 2), selection.y + (int)(background.sizeY / 1.1), selection.mobScale);
   	}
	...
	public void addButton(MobSelection selection, String message) {
   		MobBackground background = selection.getBackground();
   	...
   	public void renderEntityType(GuiGraphics graphics, EntityType type, int x, int y, int scale) {
   		MobData.get(this.menu.entity).changeRenderer(type);
   		EntityDimensions dimensions = type.getDimensions();
   		double entitySize = (25 - (10 * ((dimensions.height + dimensions.width) / 2.25))) * scale;
